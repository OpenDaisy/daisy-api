...
SUPPORTED_DEPLOYMENT_BACKENDS = ('tecs', 'zenic')
CONF = cfg.CONF
...
class Controller(controller.BaseController):
    ...
    def add_role(self, req, role_meta):
        ...
        if role_type and role_type.lower() != 'template' or not role_type:   #role_type == None or not template, cluster id must not be None
        ...
        if role_meta.has_key('nodes'):
            orig_hosts = list(eval(role_meta['nodes']))
            for role_host_id in orig_hosts:
                self._raise_404_if_host_deleted(req, role_host_id)
        
        if role_meta.has_key('services'):
            orig_services = list(eval(role_meta['services']))
            for role_service_id in orig_services:
                self._raise_404_if_service_deleted(req, role_service_id)
            role_service_id_list.extend(orig_services)

        if role_meta.has_key('config_set_id'):
            orig_config_set = str(role_meta['config_set_id'])
            self._raise_404_if_config_set_deleted(req, orig_config_set)
        ...
        if role_meta.get('glance_lv_size', None):
            if int(role_meta['glance_lv_size']) < 0:
                msg = _("glance_lv_size can't be negative.")
                raise HTTPForbidden(explanation=msg,
                                request=req,
                                content_type="text/plain")
            if not service_name_list or 'glance' not in service_name_list:
                msg = _("service 'glance' is not in role %s, so can't set the size of glance lv.") % role_meta['name']
                raise HTTPForbidden(explanation=msg,
                            request=req,
                            content_type="text/plain")
        else:
            if service_name_list and 'glance' in service_name_list:
                role_meta['glance_lv_size'] = 17100

        if role_meta.get('db_lv_size', None) and role_meta['db_lv_size']:
            if int(role_meta['db_lv_size']) < 0:
                msg = _("The size of database disk can't be negative!")
                LOG.debug(msg)
                raise HTTPForbidden(msg)
            #Only the role with service can be formulated the size of a database.
            if 'mariadb' not in service_name_list and 'mongodb' not in service_name_list:
                msg = _('The role without database service is unable to specify the size of the database!')
                LOG.debug(msg)
                raise HTTPForbidden(msg)
        else:
            if 'mariadb' in service_name_list or 'mongodb' in service_name_list:
                role_meta['db_lv_size'] = 300
            

        if role_meta.get('nova_lv_size', None):
            if role_meta['name'] != "COMPUTER":
                ...
                raise HTTPForbidden(explanation=msg,
                                    ...
                                    content_type="text/plain")
            try:
                if int(role_meta['nova_lv_size']) < 0 and int(role_meta['nova_lv_size']) != -1:
        ...
        if role_meta.has_key('deployment_backend'):
            if role_meta['deployment_backend'] not in SUPPORTED_DEPLOYMENT_BACKENDS:
                msg = "deployment backend '%s' is not supported." % role_meta['deployment_backend']
    ...
    def update_role(self, req, id, role_meta):
        ...
        role_type = role_meta.get('type', None)
        if role_type and orig_role_meta['type'].lower() != role_type.lower():
        ...
        if role_meta.has_key('cluster_id'):
            if orig_role_meta['type'].lower() == 'template':
                ...
                LOG.debug(msg)
                raise HTTPForbidden(msg)
            role_cluster = str(role_meta['cluster_id'])
        ...
        if glance_lv_value:
            ...
            if not service_name_list or 'glance' not in service_name_list:
                msg = _("service 'glance' is not in role %s, so can't set the size of glance lv.") % orig_role_meta['name']
                raise HTTPForbidden(explanation=msg,
                                request=req,
                                content_type="text/plain")
        else:
        ...
        if role_meta.get('nova_lv_size', None):
            if orig_role_meta['name'] != "COMPUTER":
                msg = _("The role is not COMPUTER, it can't set logic "
                        "volume disk for nova.")
                raise HTTPForbidden(explanation=msg,
                                    request=req,
        ...
        if role_meta.has_key('nodes') or role_host_id_list:
            ...
            for host_id in host_id_list:
                ...
                if host_disk_db_glance_nova_size['db_lv_size'] and db_lv_size and \
                    ...
                    argws['db_lv_size'] = db_lv_size
                if host_disk_db_glance_nova_size['glance_lv_size'] and glance_lv_size \
                        and int(glance_lv_size) < int(host_disk_db_glance_nova_size['glance_lv_size']):
                    argws['glance_lv_size'] = host_disk_db_glance_nova_size['glance_lv_size']
                else:
                    argws['glance_lv_size'] = glance_lv_size
                if host_disk_db_glance_nova_size['nova_lv_size'] and nova_lv_size and \
